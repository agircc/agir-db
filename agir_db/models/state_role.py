...
class StateRole(Base):
    ...
    assistant_role_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("assistant_roles.id", ondelete="CASCADE"), primary_key=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    
    # Relationships to both sides
    state: Mapped["State"] = relationship("State", back_populates="state_roles")
    role: Mapped["AssistantRole"] = relationship("AssistantRole", back_populates="role_states") 